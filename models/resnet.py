...
class BottleneckBlock(tf.keras.layers.Layer):
    ...
    def __init__(self, filters, stride, use_projection = False, se_factor = 0, **kwargs):        
        ...
        self.conv_1 = conv1x1(filters, stride, name = 'conv_0', use_bias = False)
        self.bn_1 = tf.keras.layers.BatchNormalization(name = 'bn_1')
        self.conv_2 = conv3x3(filters, 1, name = 'conv_1')
        self.bn_2 = tf.keras.layers.BatchNormalization(name = 'bn_2')
        self.conv_3 = conv1x1(filters * 4, 1, name = 'conv_2', use_bias = False)        
        self.use_projection = use_projection
        self.projection = 0
        if self.use_projection :                            
...
class ResNetBlock(tf.keras.layers.Layer):
    ...
    def __init__(self, filters,  block_size, with_reduction = False, use_bottleneck = False, se_factor = 0, **kwargs):
        ...
        if use_bottleneck:
            use_projection = True
        self.block_collector = [residual_block(filters = filters, stride = stride_0, use_projection = use_projection, se_factor = se_factor, name = 'rblock_0')]        
